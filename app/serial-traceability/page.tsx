...
export default function SerialTraceabilityPage() {
  ...
  const rows = useMemo(() => serialRecords.filter((s) => (s.serialNumber.toLowerCase().includes(query.toLowerCase()) && (project === 'ALL' || s.project === project) && (customer === 'ALL' || s.customer === customer) && (status === 'ALL' || s.status === status))), [query, project, customer, status]);

  return (
    <div>
      <SectionHeader title="Serial Traceability" subtitle="Door-in to door-out serialized audit view" />
      <FilterBar>
