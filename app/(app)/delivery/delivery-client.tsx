...
function parseUploadRows(rows: Record<string, unknown>[], direction: MovementDirection): MovementDraft[] {
  return rows
    .map((row, index) => {
      ...
      const date = normalizeCell(row, ['date', 'manifest date', 'scheduled date', 'pickup date', 'delivery date']);
      const time = normalizeCell(row, ['time', 'manifest time', 'scheduled time', 'pickup time', 'delivery time']);
      const driverCarrier = normalizeCell(row, ['driver', 'driver/carrier', 'carrier', 'driver carrier']);
      const shipmentTransferId = normalizeCell(row, ['shipment id', 'transfer id', 'shipment/transfer', 'shipment transfer id']);
      const reference = normalizeCell(row, ['reference', 'project', 'project/work order', 'work order', 'job']);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-3 md:hidden">
        {boms.map((bom) => (
          <article key={bom.id} className="erp-card p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Project / Job: {bom.project_job_number || '-'}
                </p>
              </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <div className="mt-1 text-slate-900">{formatDate(bom.bom_date)}</div>
              </div>
              <div>
                <div className="text-xs font-semibold uppercase text-slate-500">Requested By</div>
                <div className="mt-1 text-slate-900">{bom.requested_by || '-'}</div>
              </div>
            </div>
            <Link
              href={`/bom/${bom.id}`}
              className="mt-4 block rounded-md bg-cyan-700 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-cyan-800"
            >
              View / Reprint
            </Link>
          </article>
  ...
  return (
    <section className="erp-card border-slate-200 p-5">
      ...
      <div className="mt-4 rounded-md border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">
        <div className="font-semibold text-slate-700">Accepted columns</div>
...
function MovementCards({
  ...
  if (rows.length === 0) {
    return (
      <section className="erp-card p-8 text-center">
        ...
        <p className="mx-auto mt-2 max-w-xl text-sm text-slate-500">
          Use the add button or upload a CSV / Excel file to populate this section.
        </p>
      </section>
    );
  }
  ...
  return (
    <div className="space-y-3 md:hidden">
      {rows.map((row) => (
        <article key={row.id} className="erp-card p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <div>
              <div className="text-xs font-semibold uppercase text-slate-500">Date</div>
              <div className="mt-1 text-slate-900">{formatDate(row.date)}</div>
            </div>
            <div>
              <div className="text-xs font-semibold uppercase text-slate-500">Time</div>
              <div className="mt-1 text-slate-900">{formatTime(row.time)}</div>
            </div>
            <div>
              <div className="text-xs font-semibold uppercase text-slate-500">Company</div>
              <div className="mt-1 text-slate-900">{row.company || '-'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold uppercase text-slate-500">Location</div>
              <div className="mt-1 text-slate-900">{row.location || '-'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold uppercase text-slate-500">Contact</div>
              <div className="mt-1 text-slate-900">{row.contact || '-'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold uppercase text-slate-500">Driver / Carrier</div>
              <div className="mt-1 text-slate-900">{row.driverCarrier || '-'}</div>
            </div>
            <div className="col-span-2">
              <div className="text-xs font-semibold uppercase text-slate-500">Items</div>
              <div className="mt-1 text-slate-900">{row.items || '-'}</div>
            </div>
            <div className="col-span-2">
              <div className="text-xs font-semibold uppercase text-slate-500">Notes</div>
              <div className="mt-1 text-slate-900">{row.notes || '-'}</div>
            </div>
          </div>
...
function MovementTable({
  ...
  return (
    <div className="space-y-3">
      <MovementCards rows={rows} emptyMessage={emptyMessage} />
                ...
                <tbody>
                  {deliveries.map((row) => (
                    <tr key={row.id} className="border-t border-slate-200">
                      ...
                      <td className="px-3 py-2">{row.company || row.title || '-'}</td>
                      ...
                      <td className="px-3 py-2">{row.reference || row.shipmentTransferId || '-'}</td>
            ...
            {sortedMovements.map((row) => (
              <article key={row.id} className="erp-card p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-base font-semibold text-slate-900">
                      {row.manifestNumber || row.title || '(Unnamed movement)'}
                    </div>
                    <p className="mt-1 text-sm text-slate-600">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-slate-900">{row.status || '-'}</div>
                  </div>
                  <div>
                    <div className="text-xs font-semibold uppercase text-slate-500">Source</div>
                    <div className="mt-1 text-slate-900">{row.source}</div>
                  </div>
                  <div>
                    <div className="text-xs font-semibold uppercase text-slate-500">Driver / Carrier</div>
                    <div className="mt-1 text-slate-900">{row.driverCarrier || '-'}</div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-slate-900">{row.reference || row.shipmentTransferId || '-'}</div>
                  </div>
                  <div className="col-span-2">
                    <div className="text-xs font-semibold uppercase text-slate-500">Items</div>
                    <div className="mt-1 text-slate-900">{row.items || '-'}</div>
                  ...
                  <tr key={row.id}>
                    ...
                    <td>
                      {row.href ? (
                        ...
                        <span className="text-slate-500">Working row</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          <div className="mt-4 hidden md:block">
            <DataTable>
              ...
              <tbody>
                {records.map((record) => (
                  <tr key={record.id}>
                    <td className="font-semibold text-slate-900">{record.number}</td>
                    <td>{record.type}</td>
                    ...
                    <td>
                      <Link href={record.href} className="font-semibold text-cyan-700 hover:underline">
                        View / Reprint
                      </Link>
                    </td>
                  </tr>
...
export function DeliveryClient({
  ...
  async function handleUpload(file: File, direction: MovementDirection) {
    try {
      ...
      setUploadMessage(
        error instanceof Error ? error.message : 'The upload could not be read.'
      );
    }
  }
    ...
    <div className="space-y-4">
      ...
      {view === 'pickups' ? (
        <div className="space-y-4">
          <SectionToolbar
            title="Pickups"
            subtitle="Manual pickup entry plus CSV / Excel import for daily route planning."
            addLabel="Add Pickup"
            uploadLabel="Upload CSV / Excel"
            onAdd={() => {
          ...
          <UploadPanel
            ...
            description="Import pickup stops from a CSV or Excel file."
            helper="Use columns like: Date, Time, Company, Location, Contact, Items, Driver, Reference, Notes."
      ...
      {view === 'deliveries' ? (
        <div className="space-y-4">
          <SectionToolbar
            title="Deliveries"
            subtitle="Manual delivery entry plus CSV / Excel import for daily route planning."
          ...
          <UploadPanel
            ...
            description="Import delivery stops from a CSV or Excel file."
            helper="Use columns like: Date, Time, Company, Location, Contact, Items, Driver, Reference, Notes."
